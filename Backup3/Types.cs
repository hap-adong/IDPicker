//
// $Id: Types.cs 206 2010-10-21 20:14:57Z dasari $
//
...
namespace IDPicker
{
    public enum PeptideModificationStatus { IGNORE, MODIFIED, UNMODIFIED };
    /// <summary>
        ...
        public int CompareTo( ModMap other )
        {
            ...
            for( int i = 0; i < Count && compare == 0; ++i, lhsItr.MoveNext(), rhsItr.MoveNext() )
            {
                ...
                else 
                {
                    ...
                    for( int j = 0; j < lhsItr.Current.Value.Count; ++j, lhsValueItr.MoveNext(), rhsValueItr.MoveNext() )
                    {
    ...
    public class ResultList : Set<ResultInfo>
    {
        ...
        /// the result set.
        /// </summary>
        public float ResultFDR(PeptideModificationStatus status)
        {
            ...
            foreach (ResultInfo r in this)
            {
                if( r.decoyState == ResultInfo.DecoyState.DECOY)
                    numDecoys += r.spectra.Count;
                else 
                {
                    if( status == PeptideModificationStatus.MODIFIED && r.peptides.Min.mods.Count == 0 )
                        continue;
                    if( status == PeptideModificationStatus.UNMODIFIED && r.peptides.Min.mods.Count > 0 )
                        continue;
                    if (r.decoyState == ResultInfo.DecoyState.REAL)
                        numReals += r.spectra.Count;
                }
            }
            return ((float)(2.0*numDecoys))/((float)(numDecoys+numReals));
